...
def generate():
    if request.method == "POST":
        ...
        commerce = data.get("commerce") or next(
            (
                str(v).strip()
                for k, v in data.items()
                if "commerce" in k.lower() and "google" not in k.lower() and v
            ),
            "Votre Commerce",
        )
        ...
        c2_raw = data.get("c2") or next(
            (str(v).strip() for k, v in data.items() if "secondaire" in k.lower() and v),
            "violet",
        )
            ...
            if not val:
                val = next(
                    (
                        str(v).strip()
                        for k, v in data.items()
                        if f"cadeau {i}" in k.lower() and v
                    ),
                    None,
                )
